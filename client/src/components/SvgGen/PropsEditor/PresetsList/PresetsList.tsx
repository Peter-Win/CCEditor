...
export const PresetsList: React.FC<PropsPresetsList> = observer(
  ({ store }: PropsPresetsList) => {
    ...
    <div className={styles.presetsList}>
        ...
        {store.presets.map(({ name, current }, i) => (
          <tr key={name}>
            <td>
              ...
              >
                <span className={current ? styles.curName : ""}>{name}</span>
              </Radio>
            </td>
            <td>
              <Button icon={<DeleteOutlined />} disabled={current} onClick={() => setRemoveIndex(i)} />
            </td>
          </tr>
        ))}
        </tbody>
      ...
      >
        <Alert type="warning" message={`The preset ${removeIndex === null ? "" : store.presets[removeIndex]?.name} will be deleted!`} />
      </Modal>
    </div>
  )
});
